...
/**
 * @headerfile kalman_filter.cpp "kalman_filter/src/"
 * @brief kalman filter implementation file.
 *
...
/**
 * @details
 * Kalman filter class's constructor definition initializing all member variables needed for
 * kalman filter implementation.
 */
kalman_filter::kalman_filter(Eigen::MatrixXf state_matrix,             Eigen::MatrixXf control_matrix, 
			     ...
			     : m_state_matrix(state_matrix),                              
			       m_control_matrix(control_matrix),
			       ...
			       m_measurement_matrix(measurement_matrix),
 ...
 *
 * predicted_mean = A * prev_mean + B * u_t
 * predicted_cov  = A * prev_cov * A.transpose() + R_t
 *
 * K_t            = predicted_cov * C.transpose() * (C * predicted_cov * C.transpose() + Q_t).inverse()
 * corrected_mean = predicted_mean + K_t * (z_t - C * predicted_mean)
 * corrected_cov  = (I - K_t * C) * predicted_cov 
 *
 * -----------------------------------------------------------------------------------------------------
 ...
 */
std::pair<Eigen::VectorXf, Eigen::VectorXf> kalman_filter::compute_kf_corrected_state(Eigen::MatrixXf control_vector,
										      Eigen::MatrixXf sensor_measurement_vector)
{
	...
	pred_mean = calculate_kf_predicted_mean(control_vector);
	pred_cov  = calculate_kf_predicted_covariance();

	K_t       = calculate_kf_kalman_gain(pred_cov);
	corr_mean = calculate_kf_corrected_mean(pred_mean, K_t, sensor_measurement_vector);
	corr_cov  = calculate_kf_corrected_covariance(pred_cov, K_t);
 ...
 */
Eigen::VectorXf kalman_filter::calculate_kf_predicted_mean(Eigen::VectorXf control_vector)
{
	Eigen::VectorXf pred_mean = m_state_matrix * m_previous_belief_mean + m_control_matrix * control_vector;
 ...
 */
Eigen::VectorXf kalman_filter::calculate_kf_predicted_covariance()
{
	Eigen::VectorXf pred_cov = m_state_matrix * m_previous_belief_covariance * m_state_matrix.transpose() + m_state_noise_cov_vector;
 ...
 */
Eigen::MatrixXf kalman_filter::calculate_kf_kalman_gain(Eigen::VectorXf pred_cov)
{
	Eigen::MatrixXf K_t = pred_cov * m_measurement_matrix.transpose() *
	(m_measurement_matrix * pred_cov * m_measurement_matrix.transpose() + m_measurement_noise_cov_vector).inverse();
 ...
 */
Eigen::MatrixXf kalman_filter::calculate_kf_corrected_mean(Eigen::VectorXf pred_mean,Eigen::MatrixXf k_gain, Eigen::MatrixXf sensor_measu_vec)
{
	Eigen::VectorXf corr_mean = pred_mean + k_gain * (sensor_measu_vec - m_measurement_matrix * pred_mean);
 ...
 * @param [out] corrected mean as a vector.
 */
Eigen::MatrixXf kalman_filter::calculate_kf_corrected_covariance(Eigen::VectorXf pred_cov, Eigen::MatrixXf kf_gain)
{
	Eigen::MatrixXf corr_cov = ( Eigen::MatrixXf::Identity(pred_cov.rows(),pred_cov.cols()) -  kf_gain * m_measurement_matrix) * pred_cov; 
